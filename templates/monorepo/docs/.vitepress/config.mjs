...
const forgeIcon = '{{repoUrl}}'.includes('github') ? 'github' : 'gitlab'
...
export default withMermaid(
  defineConfig({
    ...
    themeConfig: {
      ...
      socialLinks: [{ icon: forgeIcon, link: '{{repoUrl}}/{{ name }}' }],
      nav: [
      ...
      footer: {
        copyright: '{{#if license}}MIT Licensed | {{/if}}Copyright © 2026-present {{authorName}}'
      }
    },
    vite: {
      optimizeDeps: {
