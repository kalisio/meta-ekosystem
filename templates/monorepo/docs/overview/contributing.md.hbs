...
git clone https://github.com/kalisio/{{ name }}.git
cd {{ name }}
...
> [!NOTE]
> **{{ name }}** follows the [standardJS](https://standardjs.com/) style guide for linting and code consistency.
...
**{{ name }}** follows the [Conventional commits specifications](https://www.conventionalcommits.org/en/v1.0.0-beta.3/)
...
{{ name }} follows [Semantic Versioning](https://semver.org/) specification. Given a version number
...
Issues can be reported in the [issue tracker](https://github.com/kalisio/{{ name }}/issues).
