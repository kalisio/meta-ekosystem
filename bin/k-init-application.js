...
try {
  ...
  const generator = plop.getGenerator('application')
  if (!generator) throw new Error('❌ Generator \'application\' not found')
  const answers = await generator.runPrompts()
  await generator.runActions({
    ...answers,
    dest: cmdDir
  })
  console.log(chalk.green(`✅ Application '${answers.name}' initialized successfully!`))
} catch (error) {
