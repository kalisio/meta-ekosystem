...
## Authenticate gh CLI
## Token is extracted from KALISIO_GITHUB_URL and passed directly
## to gh auth login — never exposed as environment variable
##
(
    set +x
    _TOKEN=$(echo "$KALISIO_GITHUB_URL" | sed 's|https://\([^@]*\)@.*|\1|')
    [[ -z "$_TOKEN" ]] && echo "-> Error: could not extract token from KALISIO_GITHUB_URL" >&2 && exit 1
